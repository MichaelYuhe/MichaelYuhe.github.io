...
**数据变化更新视图，视图变化更新数据**。其中，视图变化更新数据，可以用事件监听的方式来实现，那么如何使数据变化更新视图呢？

- 实现一个监听器`Observer`，用来劫持并监听所有属性，如果属性发生变化，就通知所有订阅者
- 实现一个订阅器`Dep`，收集订阅者，对所有监听器和订阅者进行统一管理
- 实现一个订阅者`Watcher`，可以收到属性的变化从而通知并执行相应的方法，以此来更新视图
